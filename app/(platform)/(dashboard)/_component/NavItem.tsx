...
import { Activity, CreditCard, Layout, Route, Settings } from "lucide-react";
    ...
    return (
        <AccordionItem value={organization.id} className="border-none">
            ...
            <AccordionContent className="pt-1 text-neutral-700">
                {routes.map((route) => (
                    <Button key={route.href} size="sm" onClick={() => onClick(route.href)} className={cn("w-full font-normal justify-start  mb-1", pathname === route.href && "bg-sky-500/10 text-sky-700")}
                        variant="ghost"
                    >
                        {route.icon}
                        {route.label}
